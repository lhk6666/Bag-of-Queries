...
class NordlandDataset(Dataset):
    ...
    def __init__(
        ...
        self.ground_truth = np.load(dataset_path / "Nordland_gt.npy", allow_pickle=True)
    ...
    def _validate_path(self, dataset_path):
        ...
        if not (path / "Nordland_gt.npy").is_file():
            raise FileNotFoundError(f"The file 'Nordland_gt.npy' does not exist in {path}. {msg}")
        return path