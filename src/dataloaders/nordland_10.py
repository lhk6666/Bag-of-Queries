...
class NordlandDataset_10(Dataset):
    ...
    def __init__(
        ...
        self.dataset_name = dataset_path.name + "_10"
        
        # Load image names and ground truth data
        self.dbImages = np.load(dataset_path / "Nordland_dbImages.npy")
        self.qImages = np.load(dataset_path / "Nordland_qImages.npy")
        self.ground_truth = np.load(dataset_path / "Nordland_gt_extended.npy", allow_pickle=True)
    ...
    def _validate_path(self, dataset_path):
        if dataset_path is None:
            dataset_path = Path(__file__).parent.parent.parent / "data" / "val" / "Nordland"
        ...
        if not (path / "Nordland_dbImages.npy").is_file():
            raise FileNotFoundError(f"The file 'Nordland_dbImages.npy' does not exist in {path}. {msg}")
        if not (path / "Nordland_qImages.npy").is_file():
            raise FileNotFoundError(f"The file 'Nordland_qImages.npy' does not exist in {path}. {msg}")
        if not (path / "Nordland_gt_extended.npy").is_file():
            raise FileNotFoundError(f"The file 'Nordland_gt_extended.npy' does not exist in {path}. {msg}")
        return path